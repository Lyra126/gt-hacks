...
app = FastAPI(
    ...
    description="Upload a PDF EMR file to extract structured patient data using Gemini.",
)
...
@app.post("/emr/parse-pdf/")
async def parse_emr_from_pdf(file: UploadFile = File(...)):
    """
    Accepts a PDF file, extracts its text, and uses Gemini to parse it
    into a structured EMR JSON object.
    """
    ...
    try:
        ...
        if not raw_text or raw_text.isspace():
            raise HTTPException(
                status_code=400,
                detail="Could not extract text from PDF. The file might be empty, password-protected, or a scanned image.",
            )

        # Step 2: Process the text with Gemini
        structured_data = extract_emr_details_from_text(raw_text)
        if "error" in structured_data:
            # This handles errors from the Gemini call itself
